...
public class DatasetGenerator_Pretrain_Encoded : DatasetGenerator_Encoded
{
    public DatasetGenerator_Pretrain_Encoded(Device device, ITokenizer tokenizer, List<List<int>> texts, int batchSize, int workerNum, int seqLen, int? seed = null) : base(device, tokenizer, texts, batchSize, workerNum, seqLen, seed)
    {
    }
    ...
    protected override (Tensor inputs, Tensor targets) GenerateBatch()
    {
       ...
       // var sw = System.Diagnostics.Stopwatch.StartNew();
        ...
        foreach(var index in batch)
        {
            ...
            // 截取或填充到固定长度
            var (inputSeq, targetSeq) = PrepareSequence(CollectionsMarshal.AsSpan(sentence), _seqLen);
    ...
    protected override (int[] input, int[] target) PrepareSequence(
